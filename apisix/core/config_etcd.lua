...
{ -- plugin_configs：init-etcd为增加plugin_configs目录
	"headers": {
		"Access-Control-Allow-Origin": "*",
		"X-Etcd-Index": "2007",
		"Access-Control-Allow-Methods": "POST, GET     , OPTIONS, PUT, DELETE",
...
{ -- plugin_configs，init-etcd增加plugin_configs后，执行stop.sh、start.sh后的结果
	"reason": "OK",
	"read_trailers": "function: 0x7fc026fefd90",
	"headers": {
		"Access-Control-Allow-Headers": "accept, content-type, authorization",
...
{-- upstream:
    ...
    for _, res in ipairs(res_copy) do
        ...
        elseif res.value then
            -- pre_index 不存在，且 res.value存在，则 新增数据(res.value)
            res.clean_handlers = {}
