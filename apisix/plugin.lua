...
---@type local_config_data
local local_conf

---@field plugin_metadatas config_etcd
---@class plugin_engine
local _M = {
    version         = 0.3,
...
---@param plugins_list plugins_list
local function load_plugin(name, plugins_list, is_stream_plugin)
    ...
    core.log.debug("plugin.load_plugin plugins_list",
            core.json.delay_encode(plugins_list, true))
...
---
---@alias plugin_names string[]
---@param plugin_names plugin_names
local function load(plugin_names)
    ...
    core.log.debug("plugin.load local_plugins_hash: ",
            core.json.delay_encode(local_plugins_hash, true))
...
---@param config config_etcd
function _M.load(config)
    core.log.info("plugin.load plugins: ",
    ...
    if not config then
        ...
        core.log.debug("plugin.load local_conf: ",
                core.json.delay_encode(http_plugin_names, true))
        --[[
        ["example-plugin", "batch-requests", "cors",
        "fault-injection", "ip-restriction", "limit-conn",
        "limit-count", "limit-req", "prometheus", "proxy-cache",
        "proxy-mirror", "proxy-rewrite", "redirect", "response-rewrite",
        "serverless-post-function", "serverless-pre-function", "server-info"]
        --]]
...
---@param plugins (plugin_t|plugin_conf)[]
...
---@param plugins (plugin_t|plugin_conf)[]
---@param user_route etcd_route_node_t
---@return (plugin_t|plugin_conf)[]
...
---@param route_conf etcd_route_node_t
---@param service_conf etcd_v2_node
...
---@param route_conf etcd_route_node_t
---@param consumer_conf etcd_v2_node
...
function _M.init_worker()
    ...
    core.log.debug("plugin.init_worker plugin_metadatas",
            core.json.delay_encode(plugin_metadatas, true))

    --[[
    {
        "conf_version": 0,
        "key": "\/apisix\/plugin_metadata",
        "sync_times": 0,
        "resync_delay": 5,
        "automatic": true,
        "running": true,
...
---@param api_ctx api_ctx
---@param global_rules config_etcd
